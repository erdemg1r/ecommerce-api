import { asyncHandler } from "../utils/asyncHandler.js";
import { reviewService } from "../services/reviewService.js";
import { sendNoContent, sendSuccess } from "../utils/response.js";
const list = asyncHandler(async (req, res) => {
    const { cursor, limit } = res.locals.query;
    const productId = req.params["id"];
    const { data, meta } = await reviewService.findAllByProduct(productId, cursor, limit);
    res.json({ success: true, data, meta });
});
const stats = asyncHandler(async (req, res) => {
    const result = await reviewService.getStats(req.params.id);
    sendSuccess(res, result);
});
const upsert = asyncHandler(async (req, res) => {
    const review = await reviewService.upsert(req.user.userId, req.params.id, req.body);
    sendSuccess(res, review);
});
const remove = asyncHandler(async (req, res) => {
    await reviewService.remove(req.user.userId, req.user.role, req.params.id);
    sendNoContent(res);
});
export const reviewController = {
    list,
    stats,
    upsert,
    remove,
};
//# sourceMappingURL=reviewController.js.map
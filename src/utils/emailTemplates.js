...
export const verifyEmailTemplate = (rawToken) => {
    const url = `${env.FRONTEND_URL}/api/v1/auth/verify-email?token=${rawToken}`;
    return layout("Email Adresini Doğrula", `<p>Kaydınız için teşekkürler. Email'inizi doğrulamak için aşağıdaki linke tıklayın (24 saat geçerli):</p>
...
export const resetPasswordTemplate = (rawToken) => {
    const url = `${env.FRONTEND_URL}/api/v1/auth/reset-password?token=${rawToken}`;
    return layout("Şifre Sıfırlama", `<p>Şifre sıfırlama talebi aldık. Yeni şifre belirlemek için aşağıdaki linke tıklayın (1 saat geçerli):</p>

...
const issueTokens = async (user, session) => {
...
export const authService = {
    ...
    login: async (input, session) => {
        ...
        if (!user.isVerified) {
            throw new UnauthorizedError("Email adresi doğrulanmamış. Lütfen giriş için mail adresinizi doğrulayın");
        }
    ...
    refresh: async (rawToken, session) => {
        let payload;
        try {
    ...
    me: async (userId) => {
        ...
        if (!user)
            throw new UnauthorizedError("Hesap Bulunamadı");
        return user;
    },
    listSession: async (userId) => {
    ...
    handleOAuthLogin: async (input, session) => {

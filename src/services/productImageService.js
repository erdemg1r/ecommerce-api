...
async function cleanupFiles(files) {
    await Promise.all(files.map((f) => fs.unlink(path.resolve(f.path)).catch(() => {
        //sessiz geç
    })));
}
export const productImageService = {
    addMany: async (productId, files) => {
        ...
        try {
            const created = await prisma.$transaction(files.map((f, i) => prisma.productImage.create({
                data: {
                    productId,
    ...
    remove: async (productId, imageId) => {
